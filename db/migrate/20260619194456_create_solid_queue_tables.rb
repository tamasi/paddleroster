...
# Producción separa estas tablas en su propia base (config/database.yml →
# production.queue, migrada vía db/queue_migrate con config.solid_queue.connects_to).
# En desarrollo no hay esa segunda base — viven en retroai_development como
# cualquier otra tabla de la app, vía esta migración normal. Contenido idéntico
# al que genera `bin/rails solid_queue:install` en db/queue_schema.rb (gem
# solid_queue 1.4.0), copiado a mano porque la gem no expone un generador de
# migración para el caso de una sola base.
