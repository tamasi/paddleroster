...
- **2026-06-19, post-done (reportado por Hernan probando en el navegador real):** la sección "Bot de WhatsApp" mostraba el título pero ningún contenido — ni siquiera el placeholder "Cargando estado del Bot…". Ninguna verificación previa (request specs, `curl`) lo detectó porque ninguna ejecuta JS real. Diagnóstico: se instalaron las libs nativas faltantes de Chromium (`libnspr4`, `libnss3`, `libasound2`, extraídas con `dpkg-deb -x` sin `sudo`/`apt install`, vía `LD_LIBRARY_PATH`) para correr Playwright headless contra el servidor real y ver la consola del navegador. Apareció: `Error: Matching <turbo-frame id="whatsapp-connection-status"> element has a source URL which references itself`. Causa: `app/views/configuracion/_whatsapp_connection.html.erb` (el partial que responde el fetch del frame) declaraba su propio `src:` apuntando a la misma URL que lo originó — Turbo detecta esto como referencia circular y vacía el frame en vez de reemplazarlo. **Fix:** se quitó `src:` del partial (solo el placeholder en `configuracion/show.html.erb` lo necesita, para el fetch inicial); el Stimulus controller dejó de usar `frame.reload()` (que depende de `src`) y ahora reasigna `this.element.src = this.element.dataset.pollUrl` en cada tick del polling, vía un nuevo data-attribute `poll_url` (no es el atributo `src` del frame, así que no dispara la detección de auto-referencia). Reverificado con el mismo navegador headless: estado Desconectado con botón Conectar, y estado Conectando con QR + polling, ambos sin errores de consola.
...
- 2026-06-19: Hotfix post-done reportado por Hernan probando en navegador real — la sección "Bot de WhatsApp" no mostraba nada (ni el placeholder). Causa: el partial-respuesta del turbo-frame declaraba su propio `src` apuntando a la misma URL que lo originó, y Turbo lo detecta como referencia circular y vacía el frame en vez de reemplazarlo (`Error: Matching <turbo-frame> element has a source URL which references itself`, visto recién al correr un navegador headless real contra el servidor — ningún test HTTP/request-spec lo detecta porque ninguno ejecuta JS). Fix: se quitó `src` del partial-respuesta; el Stimulus controller pasó de `frame.reload()` a reasignar `element.src` desde un nuevo data-attribute `poll_url` en cada tick. Reverificado con el mismo navegador headless en los 2 estados (Desconectado, Conectando con QR) sin errores de consola. Sin cambios de Tasks/AC — bug de implementación, no de alcance.
